...
export class SurplusPackagesService {
  ...
  async softDelete(id: string) {
    const doc = await this.surplusModel
  ...
  async findByIdForOrder(packageId: Types.ObjectId, session?: ClientSession) {
    return this.surplusModel.findById(packageId).session(session).exec();
  }
  ...
  async getSurplusPackagesForList() {
    const packages = await this.surplusModel
      .find({ isActive: true })
      .populate({
        path: 'businessId',
      ...
      packages.map(async (pkg: any) => {
        ...
        const businessReviewRate =
          totalUsersReviewedBusiness > 0
            ? reviewAggregation[0].totalRating / totalUsersReviewedBusiness
            : 0;
        ...
        return {
          ...
          discountPercent:
            pkg.discountPercent || calculateDiscount(pkg.originalPrice, pkg.offerPrice),
  ...
  async getSurplusPackageDetail(packageId: string) {
    ...
    const businessReviews = await this.reviewsService.getBusinessReviews(pkg.businessId._id);
...
type SurplusPackageLeanWithBusiness = Omit<SurplusPackage, 'businessId'> & {
  _id: Types.ObjectId;
